...
if __name__=='__main__':
    ...
    with set_rus_locale():
        df.loc[df.date_time.notnull(),'date_time'] = df.loc[df.date_time.notnull(),'date_time'].map(lambda s: datetime.strptime(s, '%d %B %H:%M %Y'))
    ...
    print(np.mean(cross_val_score(reg,x,y, cv=3, scoring='r2')))
